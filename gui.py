"""
GSAS-II Documentation Assistant — wxPython dialog.

Standalone:
    python gui.py

Integration into GSAS-II Help menu (add to GSASIIctrl.py or similar):
...
# Add project root to path so rag.py / ingest.py are importable
...
class _QueryThread(threading.Thread):
    """Run RAG query in background; post result back to wx event loop."""
    ...
    def run(self):
        try:
            import rag
            result = rag.answer_question(self.question, self.history)
...
class GSASQueryDialog(wx.Frame):
    ...
    def _build_ui(self):
        ...
        self._input = wx.TextCtrl(
            self, style=wx.TE_PROCESS_ENTER | wx.TE_MULTILINE,
            size=(-1, 54),
        )
    ...
    def _set_index_status(self, count: int):
        if count == 0:
            self._index_lbl.SetLabel("Not indexed — run: python cli.py --setup")
            self._append_system(
                "The knowledge base is empty.\n"
                "Run 'python cli.py --setup' from the Query-GSAS directory "
                "to index all GSAS-II documentation (~10 min)."
            )
        else:
    ...
    def _on_send(self, event):
        ...
        thread = _QueryThread(self, question, self._history)
        thread.start()
    ...
    def _append_user(self, text: str):
        self._chat.SetDefaultStyle(wx.TextAttr(
            _BLUE, font=wx.Font(wx.FontInfo(10).Bold())
        ))
    ...
    def _append_assistant(self, text: str):
        self._chat.SetDefaultStyle(wx.TextAttr(
            wx.BLACK, font=wx.Font(wx.FontInfo(10))
        ))
...
def show_assistant(parent=None):
    """
    Show the GSAS-II Documentation Assistant.

    Call from GSAS-II's Help menu handler:
        from gsas_query.gui import show_assistant
        show_assistant(self)

    If already open, brings the window to front.
    """
    global _instance
