...
LLM_BACKEND env var controls which LLM is used:
  "anthropic"  (default) — Anthropic Claude API (requires ANTHROPIC_API_KEY)
  "ollama"     — local Ollama server, fully on-premises (no data leaves network)

OLLAMA_MODEL defaults to "llama3" but can be any model pulled via `ollama pull`.
"""
...
def _build_messages(question: str, context: str, history: list[dict]) -> list[dict]:
    messages = []
    # Keep last 6 turns of history to limit context size
...
def _answer_ollama(messages: list[dict]) -> str:
    ...
    # Prepend system message in the messages list (Ollama chat API)
...
def answer_question(question: str, history: list[dict]) -> dict:
    ...
    if not context:
        return {
            "answer": (
                "The knowledge base is empty. Please run `python ingest.py` first "
